...
from inference.eval.seq2seq_eval_metrics import evaluate_seq2seq_predictions
from backend.utils import (_prepare_encoder_input_, convert_int2word,
                           load_a_pickle)
...
class Seq2SeqPredictions(object):
    ...
    def predict_on_features(self, features):
        ...
        result = [" ".join(convert_int2word(self.data.index2word, pred['sentence_tokens'])) for pred in predictions]
    ...
    def predict_string(self, inputs):
        ...
        predictions = [x for x in prediction_gen]
        results = [" ".join(convert_int2word(self.data.index2word, pred['sentence_tokens'])) for pred in predictions]
    ...
    def eval(self, show_first=10):
        ...
        eval_predictions = partial(evaluate_predictions,
        ...
        bleu_score = eval_predictions(predictions, show_first=show_first)
        return bleu_score
