...
from backend.model_helpers import (embedding_helper, get_available_gpus,
                                   get_forget_bias, load_embeddings,
                                   predict_words)
...
def model_fn(features, labels, mode, params):
    ...
    def make_cell(size, scope):
        with tf.variable_scope(scope):
            cell = tf.nn.rnn_cell.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(size, initializer=init), input_keep_prob=forget_bias)
        return cell
    ...
    with tf.device(GPU['titan']):
        ...
        for count, units in enumerate(num_units, 1):
            ...
            ((encoder_fw_outputs, encoder_bw_outputs),
             (encoder_fw_state, encoder_bw_state)) = tf.nn.bidirectional_dynamic_rnn(cell, cell,
                                                                                     output,
                                                                                     initial_state_fw=state_fw,
                                                                                     initial_state_bw=state_fw,
                                                                                     scope='bRNN_' + str(count),
                                                                                     dtype=tf.float32)
            output = tf.concat([encoder_fw_outputs, encoder_bw_outputs], axis=2)
    ...
    with tf.device(GPU['sidekick']):
        ...
        dec_cell = tf.nn.rnn_cell.LSTMCell(num_units=num_units[-1] * 2,  # needs to match size of last layer of encoder
